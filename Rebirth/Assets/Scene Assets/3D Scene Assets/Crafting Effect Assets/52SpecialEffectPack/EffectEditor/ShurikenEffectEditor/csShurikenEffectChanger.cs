...
public class csShurikenEffectChanger : MonoBehaviour
{
    public void ShurikenParticleScaleChange(float _Value)
    {
        ...
        foreach (ParticleSystem _ParticleSystem in ParticleSystems)
        {
            ...
            mainModule.startSpeedMultiplier *= _Value; // Updated to use startSpeedMultiplier
            mainModule.startSizeMultiplier *= _Value; // Updated to use startSizeMultiplier
            mainModule.gravityModifierMultiplier *= _Value; // Updated to use gravityModifierMultiplier

            SerializedObject _SerializedObject = new SerializedObject(_ParticleSystem);

            // Update other properties using SerializedObject
